...
#----------------------------------------------------------------------------------------
def linear_to_sRGB(rgb, use_quantile=False, clamp=True, pre_clamp=True):
...
def gamma_correction(rgb: torch.Tensor, gamma: float = 2.2, exclude_zeros=False) -> torch.Tensor:
    rgb = rgb.clamp(min=0.0, max=1.0)
    rgb_corrected = (rgb + 1e-4).pow(1. / gamma)
    if exclude_zeros:
        mask = (rgb == 0.0).all(dim=0, keepdim=True)
        return torch.where(mask, rgb, rgb_corrected)
    else:
